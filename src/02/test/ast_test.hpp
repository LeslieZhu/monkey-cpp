...
TEST(TestString, BasicAssertions)
{
	...
	letStmt->pName = std::make_unique<ast::Identifier>(tokIndent1, "myVar");
	letStmt->pValue = std::make_unique<ast::Identifier>(tokIndent2,"anotherVar");

	ast::Program program;
	program.v_pStatements.push_back(std::move(letStmt));
